...
parser.add_argument(
    "--shift_range_lat", type=float, help="range for random shift in lateral direction", default=20
)
parser.add_argument(
    "--shift_range_lon",
    type=float,
    help="range for random shift in longitudinal direction",
    default=20,
)
parser.add_argument(
    "--rotation_range", type=float, help="range for random orientation", default=180
)
...
train_loader = DataLoader(
    ...
    shuffle=True,
    pin_memory=True,
    num_workers=num_thread_workers,
    drop_last=False,
)
...
if training:
    ...
    for epoch in range(6):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_loader, 0):
            sat, grd, gt, gt_with_ori, gt_orientation, orientation_angle = [
        ...
        for i, data in enumerate(test1_loader, 0):
            ...
            for batch_idx in range(gt.shape[0]):
                ...
                distance_in_meters.append(
                    np.sqrt((loc_gt[1] - loc_pred[1]) ** 2 + (loc_gt[2] - loc_pred[2]) ** 2)
                    * test1_set.meter_per_pixel
                )
        ...
        with open(file, "ab") as f:
            np.savetxt(
                ...
                [mean_distance_error],
                fmt="%4f",
                header="test1_set_mean_distance_error_in_pixels:",
        ...
        with open(file, "ab") as f:
            np.savetxt(
                ...
                fmt="%4f",
                header="test1_set_median_distance_error_in_pixels:",
                comments=str(epoch) + "_",
            )
        ...
        print(
            "epoch: ",
            epoch,
            "mean orientation error (degrees) on test1 set: ",
        ...
        with open(file, "ab") as f:
            np.savetxt(
                ...
                [mean_orientation_error],
                fmt="%2f",
                header="test1_set_mean_orientation_error:",
                comments=str(epoch) + "_",
            )
        ...
        print(
            "epoch: ",
            epoch,
            "median orientation error (degrees) on test1 set: ",
        ...
        with open(file, "ab") as f:
            np.savetxt(
                ...
                [median_orientation_error],
                fmt="%2f",
                header="test1_set_median_orientation_error:",
                comments=str(epoch) + "_",
            )
        ...
        with open(file, "ab") as f:
            np.savetxt(
                ...
                [mean_distance_error],
                fmt="%4f",
                header="test2_set_mean_distance_error_in_pixels:",
                comments=str(epoch) + "_",
            )
        ...
        with open(file, "ab") as f:
            np.savetxt(
                ...
                [median_distance_error],
                fmt="%4f",
                header="test2_set_median_distance_error_in_pixels:",
                comments=str(epoch) + "_",
            )
        ...
        print(
            "epoch: ",
            epoch,
            "mean orientation error (degrees) on test2 set: ",
            mean_orientation_error,
        )
        ...
        with open(file, "ab") as f:
            np.savetxt(
                ...
                [mean_orientation_error],
                fmt="%2f",
                header="test2_set_mean_orientation_error:",
                comments=str(epoch) + "_",
            )
        ...
        print(
            "epoch: ",
            epoch,
            "median orientation error (degrees) on test2 set: ",
            median_orientation_error,
        )
        ...
        with open(file, "ab") as f:
            np.savetxt(
                ...
                [median_orientation_error],
                fmt="%2f",
                header="test2_set_median_orientation_error:",
                comments=str(epoch) + "_",
            )
    ...
    for i, data in enumerate(test1_loader, 0):
        ...
        for batch_idx in range(gt.shape[0]):
            ...
            gt2pred_from_north = (
                np.arctan2(np.abs(loc_gt[2] - loc_pred[2]), np.abs(loc_gt[1] - loc_pred[1]))
                * 180
                / math.pi
            )  # degree
            ...
            longitudinal_error_in_meters.append(
                pixel_distance_longitudinal * test1_set.meter_per_pixel
            )
    ...
    print(
        "percentage of samples with lateral localization error under 1m, 3m, and 5m: ",
    ...
    print(
        "percentage of samples with longitudinal localization error under 1m, 3m, and 5m: ",
    ...
    print(
        "percentage of samples with orientation error under 1 degree, 3 degrees, and 5 degrees: ",
    ...
    for i, data in enumerate(test2_loader, 0):
        ...
        for batch_idx in range(gt.shape[0]):
            ...
            pixel_distance_lateral = np.abs(np.sin(angle_diff * np.pi / 180) * pixel_distance)
            longitudinal_error_in_meters.append(
                pixel_distance_longitudinal * test2_set.meter_per_pixel
            )
