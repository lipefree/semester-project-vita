...
from router_model import CVM_VIGOR as CVM
...
label = 'old_router_loss1_subset_1v2'
learning_rate = 1e-4
...
if training is True:
    ...
    train_dataloader = DataLoader(training_set, batch_size=batch_size, shuffle=True) #To change
...
if training:
    ...
    # scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=1e-2, steps_per_epoch=len(train_dataloader), epochs=30)
    ...
    for epoch in range(30):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            curr_loss = loss.cpu().detach().numpy().mean()
        ...
        with torch.no_grad():
            ...
            writer.add_scalar("Validation/ratio_sat", current_sat_choice/(current_osm_choice + current_sat_choice), epoch)
        print(
            "epoch: ",
            epoch,
