...
import cv2
from hcnet.models.utils.utils import get_BEV_tensor, get_BEV_projection
from hcnet.models.utils.augment import train_transform


class VIGOR(Dataset):
    def __init__(self, split="train", root="/work/vita/qngo/VIGOR", same_area=True):
        ...
        if same_area:
            self.train_city_list = [
                "NewYork",
                "Seattle",
                "SanFrancisco",
                "Chicago",
            ]  # ['NewYork', 'Seattle', 'SanFrancisco', 'Chicago'] ['Seattle']
        ...
        if split == "train":
            for city in self.train_city_list:
                label_fname = os.path.join(
                    ...
                    label_root,
                    city,
                    "same_area_balanced_train.txt" if same_area else "pano_label_balanced.txt",
                )
                with open(label_fname, "r") as file:
                    for line in file.readlines():
                        ...
                        delta = np.array([data[2:4], data[5:7], data[8:10], data[11:13]]).astype(
                            float
                        )
            ...
            for city in self.test_city_list:
                label_fname = os.path.join(
                    ...
                    label_root,
                    city,
                    "same_area_balanced_test.txt" if same_area else "pano_label_balanced.txt",
                )
                with open(label_fname, "r") as file:
                    for line in file.readlines():
                        data = np.array(line.split(" "))
        ...
        self.split = split
        self.transform = None  # Look at config file
        self.center = [
        ...
        self.out = get_BEV_projection(
            pona, self.image_size, self.image_size, Fov=85 * 2, dty=0, dy=0
        )
        self.ori_noise = 0
        # self.out = None
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train" and self.transform is not None:
            pona_bev = (
                get_BEV_tensor(
                    pona, patch_size, patch_size, dty=0, dy=0, dataset=False, out=self.out
                )
                .numpy()
                .astype(np.uint8)
            )  # dataset=False get numpy HWC, else get tensor CHW
            transformed = self.transform(image=pona_bev, keypoints=self.center)
            pona_bev = transformed["image"]
            try:
                transformed_center = (
                    [transformed["keypoints"][0], transformed["keypoints"][1]]
                    if len(self.center) == 2
                    else transformed["keypoints"]
                )
            except IndexError:
            ...
            pona_bev = (
                get_BEV_tensor(pona, 500, 500, dty=0, dy=0, out=self.out).numpy().astype(np.uint8)
            )
        ...
        sat_delta[0] = (
            patch_size / 2.0 - sat_delta_init[1]
        )  # 从 [y, x] To [x, y], so fit the coord of model out
        if self.split == "train":
            transformed_center = (
                torch.tensor(transformed_center).float()
                if self.transform is not None
                else torch.tensor(self.center).float()
            )
            ...
            return (
                ...
                transformed_center,
                sat_delta,
                torch.tensor(ori_angle),
            )
        else:
            return img1, img2, pano_gps, sat_gps, torch.tensor(ori_angle), sat_delta


def fetch_dataloader(split="train"):
    train_dataset = VIGOR(split)
    ...
    if split == "train":
        ...
        print(
            "using {} images for training, {} images for validation.".format(train_size, val_size)
        )
