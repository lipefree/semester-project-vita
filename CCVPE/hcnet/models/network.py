...
class HCNet(nn.Module):
    def __init__(self, device):
        ...
        self.sat_efficientnet = EfficientNet.from_pretrained(
            "efficientnet-b0", circular=False, in_channels=intput_dim
        )
    ...
    def get_flow_now_k(self, four_point, k=4):
        ...
        gridy, gridx = torch.meshgrid(
            torch.linspace(0, w - 1, steps=w), torch.linspace(0, h - 1, steps=h), indexing="ij"
        )
        points = (
            torch.cat(
                (
        ...
        flow = torch.cat(
            (
                points_new[:, 0, :].reshape(N, w, h).unsqueeze(1),
                points_new[:, 1, :].reshape(N, w, h).unsqueeze(1),
            ),
            dim=1,
        )
        return flow, H
    ...
    def forward(self, grd, sat, sat_gps=[], iters_lev0=6, test_mode=False):
        # 0. Normalize input data
        grd = 2 * (grd / 255.0) - 1.0  # 【-1，1】
        sat = 2 * (sat / 255.0) - 1.0
        grd = grd.contiguous()
        sat = sat.contiguous()
        self.sz = grd.shape  # [N, 3, 128, 128]

        # 1. Using Backbone to Obtain Feature Maps
        grd_feature_volume, multiscale_grd = self.grd_efficientnet.extract_features_multiscale(grd)
        ...
        coords0, coords1 = self.initialize_flow_k(grd, k=self.sz[-1] // sz[-1])
        ...
        points = (
            torch.cat(
                (
                    torch.ones((1, 1)) * self.sz[3] // 2.0,
                    torch.ones((1, 1)) * self.sz[2] // 2.0,
                    torch.ones((1, 1)),
                ),
                dim=0,
            )
            .unsqueeze(0)
            .repeat(self.sz[0], 1, 1)
