...
def get_perspective_transform(src, dst):
    ...
    if not (src.shape[0] == dst.shape[0]):
        raise ValueError(
            "Inputs must have same batch size dimension. Got {}".format(src.shape, dst.shape)
        )
...
def warp_perspective(src, M, dsize, flags="bilinear", border_mode=None, border_value=0):
    ...
    if not (len(M.shape) == 3 or M.shape[-2:] == (3, 3)):
        raise ValueError("Input M must be a Bx3x3 tensor. Got {}".format(src.shape))
    # launches the warper
...
def dst_norm_to_dst_norm(dst_pix_trans_src_pix, dsize_src, dsize_dst):
    ...
    dst_norm_trans_src_norm = torch.matmul(
        dst_norm_trans_dst_pix, torch.matmul(dst_pix_trans_src_pix, src_pix_trans_src_norm)
    )
...
class HomographyWarper(nn.Module):
    ...
    def forward(self, patch_src: torch.Tensor, dst_homo_src: torch.Tensor) -> torch.Tensor:
        ...
        return F.grid_sample(
            patch_src, self.warp_grid(dst_homo_src), mode=self.mode, padding_mode=self.padding_mode
        )


def create_meshgrid(height: int, width: int, normalized_coordinates: Optional[bool] = True):
