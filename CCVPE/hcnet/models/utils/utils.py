...
from hcnet.models.utils.torch_geometry import euler_angles_to_matrix, get_perspective_transform
...
def gps2distance(Lat_A, Lng_A, Lat_B, Lng_B):
    ...
    R = torch.tensor(6371004.0).cuda()  # Earth's radius in meters
    C = torch.sin(lat_A) * torch.sin(lat_B) + torch.cos(lat_A) * torch.cos(lat_B) * torch.cos(
        lng_A - lng_B
    )
...
def get_BEV_tensor(
    img, Ho, Wo, Fov=170, dty=-20, dx=0, dy=0, dataset=False, out=None, device="cpu"
):
    # device = 'cuda' if torch.cuda.is_available() else 'cpu'
    device = device
    ...
    BEV = F.grid_sample(
        frame.permute(2, 0, 1).unsqueeze(0).float(), out.unsqueeze(0), align_corners=True
    )
    t4 = time.time()
...
def show_overlap(img1, img2, H, show=True, g_u=0, g_v=0):
    ...
    result = cv2.warpPerspective(
        image1, H, (w + w + image1.shape[1], h)
    )  # result placing image1 warped image on the left
    ...
    result[0 : image0.shape[0], image1.shape[1] : image0.shape[1] + image1.shape[1]] = (
        frame  # result placing overlap in the middle
    )
    result[0:h, image1.shape[1] + w : w + w + image1.shape[1]] = (
        image0  # result placing image0 on the right
    )
...
class Logger:
    ...
    def _print_training_status(self):
        metrics_data = [
            np.nanmean(self.running_loss_dict[k]) for k in sorted(self.running_loss_dict.keys())
        ]
    ...
    def push(self, metrics):
        ...
        for key in metrics:
            ...
            if np.isnan(metrics[key]):
                print(
                    "\033[1;91m"
                    + "There is a nan at {}!\033[0m".format(len(self.running_loss_dict[key]))
                )
...
def draw_markers(img, point, size=10, thickness=2, color=(0, 0, 255), shape=0):
    if shape == 0:
        ...
        size //= 2
        pts = np.array(
            [[point[0] - size, point[1] - size], [point[0] + size, point[1] + size]], np.int32
        )
...
class Logger_train:
    ...
    def _print_training_status(self):
        metrics_data = [
            self.running_loss[k] / self.args.SUM_FREQ for k in sorted(self.running_loss.keys())
        ]  # ['1px', '3px', 'epe', 'loss']
        training_str = "[{:6d},{:4d}/{:4d},{:10.7f}] ".format(
            self.total_steps + 1,
            self.total_steps % self.need_steps + 1,
            self.need_steps,
            self.optimizer.state_dict()["param_groups"][0]["lr"],
        )  # optimizer.state_dict()['param_groups'][0]['lr']  self.scheduler.get_last_lr()[0]
