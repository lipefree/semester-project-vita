...
from hcnet.models.utils.utils import get_homograpy, gps2distance
from hcnet.models.utils.Mercator import get_latlon_tensor, get_pixel_tensor
...
class InfoNCELoss(torch.nn.Module):
    ...
    def forward(self, sim_matrix, positive_indices):
        ...
        sim_matrix_exp = F.grid_sample(
            sim_matrix_logexp.view(batch_size, 1, h, w),
...
def corr_loss(
    ...
    if transformed_center is not None:
        corr_map = (
            corr_fn.view(batch_sz, h, w, h, w).permute(0, 3, 4, 1, 2).view(batch_sz, h * w, h, w)
        )
        transformed_center_ = transformed_center[:, :, 0] * h / sz[0]  # /4
        transformed_center_ = 2 * transformed_center_ / (corr_map.shape[-1] - 1) - 1
        corr_map = F.grid_sample(
            corr_map, transformed_center_.unsqueeze(1).unsqueeze(1), align_corners=True
        )  # [x,y]
    ...
    if len(grd_gps.shape) == 3:
        y = get_pixel_tensor(
            sat_gps[:, 0], sat_gps[:, 1], grd_gps[:, 0, 0], grd_gps[:, 0, 1], zoom, sat_size
        )
    else:
        y = get_pixel_tensor(
            sat_gps[:, 0], sat_gps[:, 1], grd_gps[:, 0], grd_gps[:, 1], zoom, sat_size
...
def vigor_gps_loss(
    ...
    y = get_pixel_tensor(
        sat_gps[:, 0], sat_gps[:, 1], grd_gps[:, 0], grd_gps[:, 1], zoom, sat_size=sat_size
    )  # get ground truth pixel coords
    ...
    for i in range(n_predictions):
        ...
        H = (
            get_homograpy(four_pred[i], sz)
            if type(four_pred) == list
            else get_homograpy(four_pred, sz)
        )
        if transformed_center is None:
            points = (
                torch.cat(
                    (
                        torch.ones((1, 1)) * sz[3] // 2.0,
                        torch.ones((1, 1)) * sz[2] // 2.0,
                        torch.ones((1, 1)),
                    ),
                    dim=0,
                )
                ...
                .repeat(sz[0], 1, 1)
                .to(grd_gps.device)
            )  # [N,2,1] only one point
        else:
            points = torch.cat(
                (
                    transformed_center,
                    torch.ones((sz[0], 1, transformed_center.shape[-1])).to(grd_gps.device),
                ),
                dim=1,
            ).to(grd_gps.device)
        x = H.bmm(points)
    ...
    est_lat, est_lon = get_latlon_tensor(
        sat_gps[:, 0], sat_gps[:, 1], x[:, 0], x[:, 1], zoom, sat_size
    )
