...
class deformable_fusion(nn.Module):
    ...
    def forward(self, osm_features, sat_features, batch_size):
        ...
        (
            ...
            fuse_feature_volume,
        ) = self.cross_da1(sat_features, osm_features, batch_size)

        fused_features = [fuse_feature_block0,
            fuse_feature_block2,
            fuse_feature_block4,
            fuse_feature_block10,
            fuse_feature_block15]
        ...
        # osm_features = self.add_skip(fused_features, osm_features)
        # sat_features = self.add_skip(fused_features, sat_features)
        ...
        (
            ...
            fuse_feature_block15,
            fuse_feature_volume,
        ) = self.cross_da2(fused_features, fused_features, batch_size)
...
class deformable_self_attention(nn.Module):
    def __init__(self, device, query_dim=256):
    ...
    def forward(self, Q):
        ...
        # Apply deformable attention for satellite and OSM
        Q_attention_output = self.deformable_attention_sat(
            query=Q,
            key=Q,
            value=Q,
