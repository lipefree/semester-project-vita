...
class CVM_VIGOR(nn.Module):
    ...
    def forward(self, grd, sat, osm):
        ...
        sat_row_chunks = torch.stack(
            list(torch.chunk(sat_feature_volume, 8, 2)), dim=-1
        ...
        for i, sat_row_chunk in enumerate(torch.unbind(sat_row_chunks, dim=-1), 0):
            sat_chunks = torch.stack(
                list(torch.chunk(sat_row_chunk, 8, 3)), dim=-1
            ...
            for j, sat_chunk in enumerate(torch.unbind(sat_chunks, dim=-1), 0):
                if j == 0:
                    sat_descriptor_row = (
                        self.sat_feature_to_descriptors(sat_chunk).unsqueeze(2).unsqueeze(3)
                    )
                else:
        ...
        for i in range(20):
            ...
            matching_score = torch.sum(
                (grd_descriptor_map1 * sat_descriptor_map_window), dim=1, keepdim=True
        ...
        for i in range(20):
            ...
            matching_score = torch.sum(
                (grd_descriptor_map2 * sat_descriptor_map_window), dim=1, keepdim=True
            ) / (sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
                matching_score_stacked2 = matching_score
            else:
                matching_score_stacked2 = torch.cat(
                    [matching_score_stacked2, matching_score], dim=1
                )
        ...
        for i in range(20):
            ...
            matching_score = torch.sum(
                (grd_descriptor_map3 * sat_descriptor_map_window), dim=1, keepdim=True
            ) / (sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
                matching_score_stacked3 = matching_score
            else:
                matching_score_stacked3 = torch.cat(
                    [matching_score_stacked3, matching_score], dim=1
                )
        ...
        for i in range(20):
            ...
            matching_score = torch.sum(
                (grd_descriptor_map4 * sat_descriptor_map_window), dim=1, keepdim=True
            ) / (sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
                matching_score_stacked4 = matching_score
            else:
                matching_score_stacked4 = torch.cat(
                    [matching_score_stacked4, matching_score], dim=1
                )
        ...
        for i in range(20):
            ...
            matching_score = torch.sum(
                (grd_descriptor_map5 * sat_descriptor_map_window), dim=1, keepdim=True
            ) / (sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
                matching_score_stacked5 = matching_score
            else:
                matching_score_stacked5 = torch.cat(
                    [matching_score_stacked5, matching_score], dim=1
                )
        ...
        for i in range(20):
            ...
            matching_score = torch.sum(
                (grd_descriptor_map6 * sat_descriptor_map_window), dim=1, keepdim=True
            ) / (sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
                matching_score_stacked6 = matching_score
            else:
                matching_score_stacked6 = torch.cat(
                    [matching_score_stacked6, matching_score], dim=1
                )
        ...
        # ori
        x_ori = torch.cat(
            [matching_score_stacked, self.sat_normalization(sat_descriptor_map)], dim=1
        )
...
class CVM_KITTI(nn.Module):
    def __init__(self, device):
        ...
        )  # no horizontal circular padding
        ...
        self.sat_feature_to_descriptors = nn.Sequential(
            nn.Flatten(start_dim=1), nn.Linear(1280 * 2 * 2, 2048)
        )
        self.sat_normalization = normalization(2, 1)
