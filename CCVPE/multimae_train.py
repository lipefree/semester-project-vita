...
label = 'multimae_localization_sat-features-rerun-rerun-pretrained-epoch-28'
...
if training:
    ...
    for epoch in range(400):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            if use_fusion_loss:
                # loss_osm, loss_in, ssim_loss, loss_grad = fusion_loss(
                #     image_vis=osm, image_ir=osm, generate_img=recons_osm.float(), i=None, labels=None
                # )
                # loss_sat, loss_in, ssim_loss, loss_grad = fusion_loss(
                #     image_vis=sat, image_ir=sat, generate_img=recons_sat.float(), i=None, labels=None
                # )
                loss_osm = masked_mse_loss(recons_osm.float(), osm, mask=None)
                loss_sat = masked_mse_loss(recons_sat.float(), sat, mask=None)
        ...
        torch.save(CVM_model.cpu().state_dict(), model_dir+'model.pth') # saving model
        ...
        with torch.no_grad(): # fix this
            ...
            for i, data in enumerate(val_dataloader, 0):
                ...
                if use_fusion_loss:
                    ...
                    loss_osm = masked_mse_loss(recons_osm, osm, mask=None)
                    loss_sat = masked_mse_loss(recons_sat, sat, mask=None)
