...
class VIGORDataset(Dataset):
    def __init__(
        ...
        for city in self.city_list:
            ...
            if self.use_osm_tiles:
                ...
                # loaded_data = np.load(osm_tile_path)['arr_0']
        ...
        dist_path = '/work/vita/qngo'
        osm_dist_path = os.path.join(dist_path, 'osm_distance', 'data.npz')
        sat_dist_path = os.path.join(dist_path, 'sat_distance', 'data.npz')
        self.osm_dist = np.load(osm_dist_path)['arr_0']
        self.sat_dist = np.load(sat_dist_path)['arr_0']
    ...
    def __getitem__(self, idx):
        ...
        if "NewYork" in self.grd_list[idx]:
            ...
            city = "Chicago"
        osm_dist = self.osm_dist[idx]
        sat_dist = self.sat_dist[idx] # osm and sat are alligned so use same index

        # osm_tile, sat = self.transform(osm_tile, sat)

        return sat_dist, osm_dist, grd, sat, osm_tile, gt, gt_with_ori, orientation, city, orientation_angle
