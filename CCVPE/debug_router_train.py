...
from new_router_model import CVM_VIGOR as CVM
...
if training:
    ...
    optimizer = torch.optim.Adam(params, lr=learning_rate, betas=(0.9, 0.999), weight_decay=1e-4)
    # optimizer = torch.optim.SGD(params, lr=1e-2)
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=1e-3, steps_per_epoch=len(train_dataloader), epochs=30)
    ...
    for epoch in range(30):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            weight = torch.tensor([1, 1.3]).to(device)
