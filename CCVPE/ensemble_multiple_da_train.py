...
from losses import infoNCELoss, cross_entropy_loss, orientation_loss, loss_ccvpe,loss_router
from ensemble_multiple_da_model import CVM_VIGOR as CVM
...
if training:
    ...
    for epoch in range(50):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            chosen_cls = chosen.argmax(dim=1)
            ...
            loss1, loss2 = loss_router(
                weight, chosen, gt_choices, sat_dist, osm_dist, t
            )
            ...
            chosen_cpu = chosen_cls.cpu().detach().numpy()
            ...
            sat_this_round = np.sum([chosen_cls.cpu().detach().numpy() < 0.5])
            global_sat_selected += sat_this_round
            global_osm_selected += bs - sat_this_round

            window_sat_selected.extend(np.array([chosen_cls.cpu().detach().numpy() < 0.5]).astype(int).flatten())
            ...
            if i % 200 == 0:    # print every 200 mini-batches
                ...
                if len(window_label) > window_size:
                    ...
                    distance_classification = distance_classification[to_cut:]
                ...
                for batch_idx in range(len(city)):
                    ...
                    if chosen_cls[batch_idx] > 0.5:
                        meter_distance = osm_dist[batch_idx]
                    else:
                        meter_distance = sat_dist[batch_idx]
                    distance_classification .append(meter_distance)

                writer.add_scalar("Train/mean_distance", np.mean(distance), global_step)
                writer.add_scalar("Train/mean_distance_classification", np.mean(distance_classification), global_step)
                writer.add_scalar(
        ...
        with torch.no_grad(): # fix this
            for i, data in enumerate(val_dataloader, 0):
                ...
                loss_validation += torch.mean(loss_validation_1 * lambd)
                ...
                chosen_cls = chosen_cls.cpu().detach().numpy()

                chosen_t = np.array([chosen_cls > 0.5]).astype(int).flatten()
            ...
            writer.add_scalar("Validation/mean_distance_classification", mean_distance_error, epoch)
            writer.add_scalar("Validation/median_distance_classification", median_distance_error, epoch)
