...
if training is True:
    ...
    train_indices = index_list[0: int(len(index_list)*0.0003)]
...
def load_model(model, model_name, epoch_str):
...
if training:
    ...
    print("recording distances of ", osm_model_name)

    # self.osm_model = self.load_model(self.osm_model, osm_model_name, osm_epoch)
    CVM_model = load_model(CVM_model, osm_model_name, osm_epoch)
    ...
    dist = np.zeros(len(train_indices))
    ...
    for epoch in range(1):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(tqdm(train_dataloader), 0):
            ...
            for batch_idx in range(len(city)):
                ...
                print(f'logged distance {meter_distance} at position {data_index}')
                
                dist[data_index] = meter_distance
                data_index += 1
        ...
        print(f'data length was {data_index}')
        print(f'number of non zero entry is {np.count_nonzero(dist)}')
        print(f'mean dist {np.mean(dist)}')

        dist_path = '/work/vita/qngo/osm_distance_test'

        np.savez(os.path.join(dist_path, 'data.npz'), dist)
