...
def main():
    weight_ori = 1e1
    weight_infoNCE = 1e4
    experiment_name = 'random_hard_select_fusion'
    ...
    optimizer = torch.optim.AdamW(model_wrapper.get_model().parameters(),
    ...
    if debug:
        num_epoch *= 100
    best_epoch = train(num_epoch, train_dataloader, val_dataloader,
                       optimizer, model_wrapper, writer, device, experiment_name, debug)
...
def test(experiment_name, epoch, dataset_root, device, tb_writer, area='samearea', pos_only=True):
    ...
    tb_writer.add_scalar("Test/mean_distance",
                         np.mean(distances),
                         0)
    tb_writer.add_scalar("Test/median_distance",
                         np.median(distances),
                         0)


def train(num_epoch, train_dataloader, val_dataloader, optimizer, model_wrapper, tb_writer, device, experiment_name, debug):
    global_step = 0
    print('train')
