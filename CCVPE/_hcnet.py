...
def fetch_optimizer(args, model):
    ...
    if args.scheduler == "Cosine":
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 85)
    elif args.scheduler == "Reduce":
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, mode="min", factor=0.8, min_lr=0.000005, patience=0, eps=args.epsilon
        )
        ...
        scheduler = optim.lr_scheduler.CyclicLR(
            optimizer,
            base_lr=0.000005,
            max_lr=args.lr,  # 9000  171000
            step_size_up=9000,
            step_size_down=171000,
            mode="triangular2",
            cycle_momentum=False,
...
def train(args):
    ...
    nw = min(
        [os.cpu_count(), args.batch_size if args.batch_size > 1 else 0, 12]
    )  # number of workers
    print(
        "Using {} dataloader workers every process".format(nw)
    )  # https://blog.csdn.net/ResumeProject/article/details/125449639
    train_loader = DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=True, pin_memory=True, num_workers=nw
    )
    val_loader = DataLoader(
        val_dataset, batch_size=32, shuffle=False, pin_memory=True, num_workers=nw
    )
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            image1, image2, grd_gps, sat_gps, transformed_center, sat_delta, ori_angle = [
                x.cuda() for x in data_blob
            ]  # img1, img2, pona_gps, sat_gps
            sat_delta = sat_delta if args.orig_label else None

            # Forward pass
            four_pred, corr_fn = model(
                image1, image2, sat_gps=sat_gps.float(), iters_lev0=args.iters_lev0
            )
            loss, metrics = vigor_gps_loss(
                four_pred,
                grd_gps=grd_gps,
                sat_gps=sat_gps,
                args=args,
                ...
                ori_angle=ori_angle,
                w3=w3,
                orien=args.dataset == "vigor" and args.orien,
                transformed_center=transformed_center,
                sz=[image1.shape[2], image1.shape[3]],
                gamma=args.gamma,
            )
            loss2 = corr_loss(
                grd_gps,
            ...
            scaler.step(
                optimizer
            )  # https://blog.csdn.net/weixin_51723388/article/details/126260788
        ...
        print(
            "\033[1;94m"
            + "Epoch: [{}/{}], Loss: {}. \033[0m".format(epoch + 1, num_epochs, val_mdis.item())
        )
        if val_mdis < best_dis:
            ...
            print(
                "\033[1;91m" + "Val has no improvement vs {}!\033[0m".format(best_dis)
            )  # https://blog.csdn.net/qq_63167347/article/details/125824913
        with open(logger.file_name, "a") as file:
            file.write(
                "Epoch: [{}/{}], mdis: {}, the best: {}, at {}.".format(
                    epoch + 1, num_epochs, val_mdis.item(), best_dis, checkpoint["steps"]
                )
                + "\n"
            )
        epoch += 1
