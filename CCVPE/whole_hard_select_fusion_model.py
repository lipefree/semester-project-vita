...
class CVM_VIGOR(nn.Module):
    def __init__(
        ...
        self.router_volume = PatchRouter(1280 * 16 * 16, patch_size=16)
        self.router15 = PatchRouter(320 * 16 * 16, patch_size=16)
        self.router10 = PatchRouter(112 * 32 * 32, patch_size=32)
        self.router4 = PatchRouter(40 * 64 * 64, patch_size=64)
        self.router2 = PatchRouter(24 * 128 * 128, patch_size=128)
        self.router0 = PatchRouter(16 * 256 * 256, patch_size=256)
    ...
    def forward(self, grd, sat, osm, scores, temp=0.1):
        ...
        fusion_feature_block4, weights4 = self.router4(
            sat_feature_block4, osm_feature_block4, temp
        )
        fusion_feature_block2, weights2 = self.router2(
            sat_feature_block2, osm_feature_block2, temp
        )
        fusion_feature_block0, weights0 = self.router0(
            sat_feature_block0, osm_feature_block0, temp
        )
