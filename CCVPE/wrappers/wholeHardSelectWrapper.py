from whole_hard_select_fusion_model import CVM_VIGOR as CVM
...
class WholeHardSelectWrapper:
    ...
    def infer(self, data, training=False):
        ...
        # we schedule the temp as it apparently stabilize the training, we go from
        # temp=2 to temp=0.1, where at 30k global steps we are at 0.1.
        if self.schedule_temp:
            # 0 2
            # 30'000 0.1 => a*30'000 + 2 = 0.1
            a = -1.9 / 30000
            temp_f = a * self.global_step + 2
            temp = max(temp_f, 0.1)  # we do no to lower than 0.1 for stability
        else:
            temp = 0.1

        output = self.model(grd, sat, osm, scores=None, temp=temp)
    ...
    def validation_step(self, data, validation_state):
        ...
        validation_state["mean_choices"].append(self.get_choices(output))
    ...
    def validation_log(self, validation_state, epoch, writer):
        ...
        writer.add_scalar(
            "Validation/mean_choices",
            np.median(validation_state["mean_choices"]),
            epoch,
        )
