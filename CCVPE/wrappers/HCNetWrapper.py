from hcnet.models.network import HCNet
...
import random
from hcnet.models.utils.utils import get_BEV_projection, get_BEV_tensor
...
class HCNetWrapper:
    def __init__(
        ...
        self.model = HCNet(device).to(device)
    ...
    def infer(self, data, training=False):
        grd, sat, osm, gt, gt_with_ori, gt_orientation, city, gt_flattened = data
        out = get_BEV_projection(grd, self.image_size, self.image_size, Fov=85 * 2, dty=0, dy=0)
        grd_bev = (
            get_BEV_tensor(
                grd, self.image_size, self.image_size, dty=0, dy=0, dataset=False, out=out
            )
            .numpy()
            .astype(np.uint8)
        )  # dataset=False get numpy HWC, else get tensor CHW
        print("grd bev shape ", grd_bev.shape)

        output = self.model(grd_bev, sat)
