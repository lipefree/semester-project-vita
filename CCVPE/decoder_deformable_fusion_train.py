...
from decoder_deformable_fusion_model import CVM_VIGOR as CVM
...
label = 'fused_image_ccvpe_v2_masked_ratio-82_l2_loss_separate-reconstruction'
...
if training:
    ...
    for epoch in range(500):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            output = CVM_model(grd, sat, osm, use_mask=True)
            ...
            if use_fusion_loss:
                ...
                loss_osm = masked_mse_loss(recons_osm.float(), osm)
                loss_sat = masked_mse_loss(recons_sat.float(), sat)

                # loss += loss_fusion*100
                loss += loss_osm
                loss += loss_sat
            ...
            if global_step % 10000 == 1:
                ...
                sat = invTrans(sat)
                osm = invTrans(osm)
                fused_image = invTrans(fused_image)
                ...
                for idx in range(len(city)):
                    ...
                    save_image(recons_osm[idx], os.path.join(im_path, f"recons_osm_image_{idx}.png"))
                    save_image(recons_sat[idx], os.path.join(im_path, f"recons_sat_image_{idx}.png"))
        ...
        with torch.no_grad(): # fix this
            for i, data in enumerate(val_dataloader, 0):
                ...
                if use_fusion_loss:
                    ...
                    loss_fusion = loss_osm + loss_sat
                    # loss += loss_fusion*100
                    loss_validation += loss_osm
                    loss_validation += loss_sat

                    # loss_validation += loss_fusion*100
                ...
                running_loss_image_validation.append(loss_fusion.item())
            ...
            writer.add_scalar("Validation/loss_image", np.mean(running_loss_image_validation), epoch)
