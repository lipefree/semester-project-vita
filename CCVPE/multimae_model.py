...
from model_utils.multimae import MultiMAE, MultiViT, pretrain_multimae_base
from model_utils.input_adapters import PatchedInputAdapter
from model_utils.output_adapters import SpatialOutputAdapter, DPTOutputAdapter
...
from model_utils.multimae_utils import build_2d_sincos_posemb
...
class CVM_VIGOR(nn.Module):
    def __init__(self, device, circular_padding, use_adapt, use_concat, use_mlp=False,
        ...
        self.use_sat_features = use_sat_features
        ...
        self.multimae_encoder = MultiMaeEncoder()

        epoch = 28
        self.multimae_encoder.load_multimaae_weights(f'/work/vita/qngo/models/VIGOR/multimae_encoder_new-save/{epoch}/model.pt')
        ...
        self.multivit = MultiViT(
            input_adapters=self.input_adapters,
            output_adapters=self.output_adapters
        )

        self.mae_conv_1 = nn.Conv2d(256, 24, kernel_size=1)
        self.mae_conv_2 = nn.Conv2d(256, 40, kernel_size=1)
        self.mae_conv_3 = nn.Conv2d(256, 112, kernel_size=1)
        self.mae_conv_4 = nn.Conv2d(256, 320, kernel_size=1)
        self.mae_conv_volume = nn.Conv2d(256, 1280, kernel_size=1)
        self.osm_out = nn.Conv2d(16, 3, kernel_size=1)
        self.sat_out = nn.Conv2d(16, 3, kernel_size=1)

        self.conv_block = nn.Sequential(
            nn.Conv2d(768, 1280, kernel_size=1)
        )
        self.block15 = nn.Sequential(
            nn.Conv2d(768, 320, kernel_size=1)
        )
        self.block10 = nn.Sequential(
            nn.Upsample(size=(32, 32), mode='bilinear'),
            nn.Conv2d(768, 112, kernel_size=1)
        )
        self.block4 = nn.Sequential(
            nn.Upsample(size=(64, 64), mode='bilinear'),
            nn.Conv2d(768, 40, kernel_size=1)
        )
        self.block2 = nn.Sequential(
            nn.Upsample(size=(128, 128), mode='bilinear'),
            nn.Conv2d(768, 24, kernel_size=1)
        )
        self.block0 = nn.Sequential(
            nn.Upsample(size=(256, 256), mode='bilinear'),
            nn.Conv2d(768, 16, kernel_size=1)
        )

        # image size (512) / (stride * P_H)
        h_posemb = 7
        w_posemb = 7

        self.pos_emb = build_2d_sincos_posemb(h=h_posemb, w=w_posemb, embed_dim=768)
    ...
    def forward(self, grd, sat, osm):
        ...
        x = {'osm':osm, 'sat':sat}
        preds = self.multivit(x, return_all_layers=True)
        out_osm, path_1_osm, path_2_osm, path_3_osm, path_4_osm = preds['osm']
        out_sat, path_1_sat, path_2_sat, path_3_sat, path_4_sat = preds['sat']

        if self.use_sat_features:
            out, path_1, path_2, path_3, path_4 = preds['sat']
        else:
            out, path_1, path_2, path_3, path_4 = preds['osm']

        fuse_feature_volume = self.mae_conv_volume(path_4)
        fuse_feature_block0 = out # [16, 256, 256]
        fuse_feature_block2 = self.mae_conv_1(path_1) #[24, 128, 128]
        fuse_feature_block4 = self.mae_conv_2(path_2) # [40, 64, 64]
        fuse_feature_block10 = self.mae_conv_3(path_3) # [112, 32, 32]
        fuse_feature_block15 = self.mae_conv_4(path_4) # [320, 16, 16]
        recons_osm = F.interpolate(self.osm_out(out_osm), size=(512, 512), mode='bilinear', align_corners=False)
        recons_sat = F.interpolate(self.sat_out(out_sat), size=(512, 512), mode='bilinear', align_corners=False)
        masks = None
        # volume shape : ([8, 1280, 16, 16])
        fuse_feature_blocks = [
    ...
    def add_pos_emb(self, x, size=16):
