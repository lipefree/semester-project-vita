...
class CVM_VIGOR(nn.Module):
    def __init__(self, device, circular_padding, use_adapt, use_concat, use_mlp=False):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(8192, 1024),  # Input size = 8192 (H*W*2), output size = 1024
            nn.ReLU(),
            nn.Linear(1024, 256),  # Output size = 256
            nn.ReLU(),
            nn.Linear(256, 1),  # Output size = 1 (example, can be adjusted as needed)
            nn.Sigmoid()
        )
    ...
    def forward(self, grd, sat, osm):
        ...
        max_pooled_osm = torch.max(osm_feature_block4, dim=1)[0]
        max_pooled_osm = max_pooled_osm.flatten(1)
        
        max_pooled_sat = torch.max(sat_feature_block4, dim=1)[0].flatten(1)
        concat = torch.concat((max_pooled_osm, max_pooled_sat), dim=1)
        
        coeff = self.mlp(concat)
        if self.hard_gate:
            gate_coeff = (coeff > 0.5).float()
        else:
            gate_coeff = coeff

        gate = gate_coeff.unsqueeze(2).unsqueeze(3)  # Shape: [B, 1, 1, 1]
        fuse_feature_volume = gate*osm_feature_volume + (1 - gate)*sat_feature_volume

        dims = [15, 10, 4, 2, 0]
        fuse_feature_blocks = []

        for i in dims:
            fuse_feature_blocks.append(gate*multiscale_osm[i] + (1 - gate)*multiscale_sat[i])

        fuse_feature_blocks.append(None)
