...
from model_utils.unet_fusion import unet_fusion
...
class CVM_VIGOR(nn.Module):
    def __init__(self, device, circular_padding, use_adapt, use_concat, use_mlp=False):
        ...
        self.unet_fusion = unet_fusion(self.device)

        input_dim = 1280
        # hidden_dim = 640
        hidden_dim = 512
        self.global_pool = nn.AdaptiveAvgPool2d((1, 1))  # Reduce to (B, 1280, 1)
    ...
    def forward(self, grd, sat, osm):
        ...
        (
            ...
            fuse_feature_volume, # 1280, 16, 16
        ) = self.unet_fusion(sat_features, osm_features, batch_size)

        avg_pooling = self.global_pool(fuse_feature_volume)
        ...
        return (chosen, logits_flattened, heatmap, x_ori) + tuple(matching_score_stacked_list)
